"""Standalone crawler CLI runner.

This module keeps crawler standalone actions separate from daemon runtime wiring.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--run-api",
        action="store_true",
        help="Start crawler daemon runtime (compatibility flag; prefer daemon/main.py).",
    )
    parser.add_argument(
        "--api-host",
        default="127.0.0.1",
    ...
    parser.add_argument(
        "--api-debug",
        action="store_true",
        help="Reserved compatibility flag for --run-api.",
    )
    return parser
...
def run_cli() -> int:
    ...
    if (
        not args.run_api
        and not args.url_to_canonicalize
        ...
        print(
            "No action selected. Use --run-api, --canonicalize, --ingest-demo, "
            "--extract-links-demo, --frontier-demo, --crawl-once-demo, --check-url, or --fetch-url."
        )
        return 0
    ...
    if args.run_api:
        ...
        print("[compat] --run-api is deprecated; prefer pa1/crawler/src/daemon/main.py")
        run_daemon_runtime()
        return 0
